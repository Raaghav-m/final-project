...
contract DAOManager {
    ...
    // Constructor
    constructor(IPolygonZkEVMBridge _polygonZkEVMBridge, address _pingReceiver) {
        userSideAdmin = msg.sender;
        pingReceiver = _pingReceiver;
    ...
    function getAllDaos() public view returns (dao[] memory) {
        dao[] memory tempDaos = new dao[](totalDaos);
        for (uint256 i = 1; i <= totalDaos; i++) {
            tempDaos[i - 1] = daoIdtoDao[i];
        }           
        return tempDaos;
    }
}